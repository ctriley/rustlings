...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let red = u8::try_from(tuple.0).ok().unwrap();
        let green = u8::try_from(tuple.1).ok().unwrap();
        let blue = u8::try_from(tuple.2).ok().unwrap();
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let red = u8::try_from(arr[0]).ok().unwrap();
        let green = u8::try_from(arr[1]).ok().unwrap();
        let blue = u8::try_from(arr[2]).ok().unwrap();
        Ok(Color { red: red, green: green, blue: blue})
     }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            Err(String::from("Slice not correct size"))
        } else {
            let red = u8::try_from(slice[0]).ok().unwrap();
            let green = u8::try_from(slice[1]).ok().unwrap();
            let blue = u8::try_from(slice[2]).ok().unwrap();
            Ok(Color { red: red, green: green, blue: blue})
        }
    }
}
