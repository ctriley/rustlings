...
mod tests {
    ...
    fn result_with_list() {
        ...
        let x : Result<Vec<i32>, DivisionError> = Ok(division_results.map(Result::unwrap).collect());
        //let x = Ok(division_results.map(Result::unwrap).collect::<Result<Vec<i32>, DivisionError>>());
